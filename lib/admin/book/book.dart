...
class _BookPageState extends State<BookPage> {
          ...
          builder: (context, snapshot) {
            ...
            return FutureBuilder<List<String>>(
              future: Future.wait(fullList.map((book) async {
                DocumentSnapshot bookTypeSnapshot =
                    await book.book.bookTypeRef.get();
                Map<String, dynamic> bookTypeSnapshotData =
                    bookTypeSnapshot.data() as Map<String, dynamic>;
                return bookTypeSnapshotData["name"];
              })),
              builder: (context, futureBookTypeNames) {
                if (!futureBookTypeNames.hasData)
                ...
                return Column(children: [
                  ...
                  ElevatedButton(
                    onPressed: () {
                      String keyword =
                          txtSearchKeyword.text.trim().toLowerCase();
                      setState(() {
                        filteredList = fullList
                            .where((book) =>
                                book.book.bookTypeRef == bookType!.ref)
                            .toList();
                              ...
                              Expanded(
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Text(
                                          "Tên sách: ${bookSnapshot.book.name}",
                                          style: TextStyle(
                                              fontWeight: FontWeight.bold),
                                        ),
                                        Text(
                                            "Giá: ${formatCurrency.format(bookSnapshot.book.price)}"),
                                        Text(
                                            "Số lượng: ${bookSnapshot.book.quantity.toString()}"),
  ...
  void fetchBookTypes() async {
    Stream<List<BookTypeSnapshot>> bookTypeSnapshots =
        BookTypeSnapshot.getAll();
